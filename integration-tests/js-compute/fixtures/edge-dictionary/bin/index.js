...
// Dictionary
{
  routes.set("/dictionary-store/exposed-as-global", () => {
    let error = assert(typeof Dictionary, 'function', `typeof Dictionary`)
    if (error) { return error }
    return pass()
  });
  routes.set("/dictionary-store/interface", dictionaryInterfaceTests);
  // Dictionary constructor
  {

    routes.set("/dictionary-store/constructor/called-as-regular-function", () => {
    ...
    routes.set("/dictionary-store/constructor/parameter-calls-7.1.17-ToString", () => {
      let sentinel = Symbol();
      const test = () => {
        const name = {
          toString() {
    ...
    routes.set("/dictionary-store/constructor/empty-parameter", () => {
      let error = assertThrows(() => {
        new Dictionary()
    ...
    routes.set("/dictionary-store/constructor/found-store", () => {
      const store = createValidDictionary()
      let error = assert(store instanceof Dictionary, true, `store instanceof Dictionary`)
      if (error) { return error }
      return pass()
    });
    routes.set("/dictionary-store/constructor/missing-store", () => {
      let error = assertThrows(() => {
        new Dictionary('missing')
      }, Error, `Dictionary constructor: No Dictionary named 'missing' exists`)
      if (error) { return error }
      return pass()
    });
    routes.set("/dictionary-store/constructor/invalid-name", () => {
  ...
  {
    routes.set("/dictionary-store/get/called-as-constructor", () => {
      let error = assertThrows(() => {
    ...
    routes.set("/dictionary-store/get/called-unbound", () => {
      let error = assertThrows(() => {
    ...
    // https://tc39.es/ecma262/#sec-tostring
    routes.set("/dictionary-store/get/key-parameter-calls-7.1.17-ToString", () => {
      let sentinel = Symbol();
      const test = () => {
        const key = {
          toString() {
    ...
    routes.set("/dictionary-store/get/key-parameter-not-supplied", () => {
      let error = assertThrows(() => {
        const store = createValidDictionary()
        store.get()
    ...
    routes.set("/dictionary-store/get/key-parameter-empty-string", () => {
      let error = assertThrows(() => {
        const store = createValidDictionary()
        store.get('')
    ...
    routes.set("/dictionary-store/get/key-parameter-255-character-string", () => {
    ...
    routes.set("/dictionary-store/get/key-parameter-256-character-string", () => {
      let error = assertThrows(() => {
        const store = createValidDictionary()
    ...
    routes.set("/dictionary-store/get/key-does-not-exist-returns-null", () => {
      let store = createValidDictionary()
      let result = store.get(Math.random())
      error = assert(result, null, `store.get(Math.random())`)
      if (error) { return error }
      return pass()
    });
    routes.set("/dictionary-store/get/key-exists", () => {
      let store = createValidDictionary()
      let result = store.get('twitter')
      error = assert(result, "https://twitter.com/fastly", `store.get('twitter') === "https://twitter.com/fastly"`)
      if (error) { return error }
      return pass()
    });
  }
...
// TODO: Implement ReadableStream getIterator() and [@@terator]() methods
...
function assertRejects(func, errorClass, errorMessage) {
  try {
    func()
