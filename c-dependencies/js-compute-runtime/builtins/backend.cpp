...
std::map<std::string, Cipher> CIPHER{
    ...
    {"ECDHE-RSA-AES128-SHA",
     Cipher(std::string("ECDHE-RSA-AES128-SHA"), KeyExchange::EECDH, Authentication::RSA,
            Encryption::AES128, MessageDigest::SHA1, Protocol::TLSv1, EncryptionLevel::HIGH, 128)},
    {"ECDHE-RSA-AES256-SHA",
     Cipher(std::string("ECDHE-RSA-AES256-SHA"), KeyExchange::EECDH, Authentication::RSA,
            Encryption::AES256, MessageDigest::SHA1, Protocol::TLSv1, EncryptionLevel::HIGH, 256)},
...
void OpenSSLCipherConfigurationParser::init(std::map<std::string, std::vector<Cipher>> *aliases) {
  ...
  // suites are available. Note: these cipher strings do not change the negotiated version of SSL or
  ...
  // anonymous ciphers. Note that this rule does not cover eNULL, which is not included by ALL (use
  // COMPLEMENTOFALL if necessary). Note that RC4 based cipher suites are not supported by Fastly
  // and the only supported anonymous ciphers are `ecdh` and `triple_des`.
...
void OpenSSLCipherConfigurationParser::moveToEnd(std::vector<Cipher> *ciphers,
                                                 std::vector<Cipher> *ciphersToMoveToEnd) {
...
void OpenSSLCipherConfigurationParser::moveToStart(std::vector<Cipher> *ciphers,
                                                   std::vector<Cipher> *ciphersToMoveToStart) {
...
void OpenSSLCipherConfigurationParser::strengthSort(std::vector<Cipher> *ciphers) {
  /*
 ...
 */
std::vector<Cipher> OpenSSLCipherConfigurationParser::defaultSort(std::vector<Cipher> *ciphers) {
  ...
  auto aes = {Encryption::AES128, Encryption::AES128GCM, Encryption::AES256, Encryption::AES256GCM};
...
std::vector<Cipher>
OpenSSLCipherConfigurationParser::filterByStrengthBits(std::vector<Cipher> *ciphers,
                                                       int strength_bits) {
  std::vector<Cipher> result;
  for (auto cipher : *ciphers) {
...
std::vector<Cipher>
OpenSSLCipherConfigurationParser::filterByProtocol(std::vector<Cipher> *ciphers,
...
std::vector<Cipher>
OpenSSLCipherConfigurationParser::filterByKeyExchange(std::vector<Cipher> *ciphers,
                                                      std::set<KeyExchange> kx) {
  return filter(ciphers, std::nullopt, kx, std::nullopt, std::nullopt, std::nullopt, std::nullopt);
}

std::vector<Cipher>
OpenSSLCipherConfigurationParser::filterByAuthentication(std::vector<Cipher> *ciphers,
                                                         std::set<Authentication> au) {
  return filter(ciphers, std::nullopt, std::nullopt, au, std::nullopt, std::nullopt, std::nullopt);
}

std::vector<Cipher>
OpenSSLCipherConfigurationParser::filterByEncryption(std::vector<Cipher> *ciphers,
                                                     std::set<Encryption> enc) {
  return filter(ciphers, std::nullopt, std::nullopt, std::nullopt, enc, std::nullopt, std::nullopt);
}

std::vector<Cipher>
OpenSSLCipherConfigurationParser::filterByEncryptionLevel(std::vector<Cipher> *ciphers,
...
std::vector<Cipher>
OpenSSLCipherConfigurationParser::filterByMessageDigest(std::vector<Cipher> *ciphers,
                                                        std::set<MessageDigest> mac) {
  return filter(ciphers, std::nullopt, std::nullopt, std::nullopt, std::nullopt, std::nullopt, mac);
}

std::vector<Cipher> OpenSSLCipherConfigurationParser::filter(
    std::vector<Cipher> *ciphers, std::optional<std::set<Protocol>> protocols,
...
std::vector<std::string> OpenSSLCipherConfigurationParser::split(std::string s,
...
OpenSSLCipherConfigurationParser::splitCipherSuiteString(std::string string) {
...
std::vector<Cipher> OpenSSLCipherConfigurationParser::parse(std::string_view expression) {
  /**
