...
bool Dictionary::get(JSContext *cx, unsigned argc, JS::Value *vp) {
  ...
  size_t nwritten = 0;
  int status = xqd_dictionary_get(Dictionary::dictionary_handle(self), name_chars.get(), name_len,
...
bool Dictionary::constructor(JSContext *cx, unsigned argc, JS::Value *vp) {
  ...
  std::string name(name_chars.get(), name_len);
  ...
  auto status = xqd_dictionary_open(name.c_str(), name_len, &dict_handle);
  std::cout << "status: " << status << "\n";
  std::flush(std::cout);
  if (status == FastlyStatus::badf) {
    JS_ReportErrorNumberASCII(cx, GetErrorMessage, nullptr,
                              JSMSG_DICTIONARY_DOES_NOT_EXIST, name.c_str());
    return false;
  }
  if (!HANDLE_RESULT(cx, status)) {
    return false;
